  ...
  ALL_TABLES.forEach(k => {
    ...
    DATA[k].forEach(row => {
      if (row.usage !== null && row.usage !== undefined && row.usage !== '') {
        ...
        if (!isNaN(u)) {
          if (u > 1 && u <= 100) { row.usage = u / 100; fixed.rows++; tableFixed = true; }
          else if (u < 0) { row.usage = 0; fixed.rows++; tableFixed = true; }
        }
      }
    });
...
function showDataHealthReport(report) {
  ...
  const fixableUsage = r.issues.filter(x => x.code === 'USAGE_INVALID').length;
  ...
  const fixableTotal = fixableSeq + fixableUsage + fixableNeg + fixableDup + fixableEmpty + r.issues.filter(x => x.code === 'SUBTOTAL_INCONSISTENT').length;
  let fixBtns = '';
  ...
  if (fixableUsage > 0) fixBtns += '<button class="btn btn-sm" onclick="autoFixUsageRates(); document.getElementById(\'healthOverlay\').remove(); setTimeout(function(){ showDataHealthReport(); },400);">修正利用率 (' + fixableUsage + ')</button>';
  ...
  overlay.innerHTML = '<div class="health-modal">' +
    ...
    (hasFixable ? '<button class="btn primary" onclick="autoFixEmptyRows(); autoFixMissingSeqs(); autoFixUsageRates(); autoFixNegativeValues(); autoFixDuplicateSeqs(); fixAllConsistency(); document.getElementById(\'healthOverlay\').remove(); setTimeout(function(){ showDataHealthReport(); },500);">🔧 全部修复</button>' : '') +
    '</div></div>';
